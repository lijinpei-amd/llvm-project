...
define amdgpu_ps void @buffer_load_lds(
  ...
  ; CHECK:   BUFFER_LOAD_DWORD_LDS_OFFSET {{.*}} :: (dereferenceable load (s32) from `ptr addrspace(8) poison`, align 1, addrspace 8), (dereferenceable store (s32) into %ir.src, align 1, addrspace 3)
...
define amdgpu_ps void @global_load_lds(
  ...
  ; CHECK: bb.1 (%ir-block.0):
  ; CHECK:   GLOBAL_LOAD_LDS_DWORD {{.*}} :: (load (s32) from `ptr addrspace(1) poison` + 16, align 1, addrspace 1), (store (s32) into %ir.src + 16, addrspace 3)
