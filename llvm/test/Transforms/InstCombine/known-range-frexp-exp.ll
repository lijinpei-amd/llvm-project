...
; The exponent is unspecified for nan/inf, so the range can only be used to fold
; away the clamp when the source is known to never be nan or inf.
...
define i32 @frexp_f32_clamp_max_no_nan_inf(float nofpclass(nan inf) %x) {
; CHECK-LABEL: define i32 @frexp_f32_clamp_max_no_nan_inf(
; CHECK-SAME: float nofpclass(nan inf) [[X:%.*]]) {
; CHECK-NEXT:    [[FREXP:%.*]] = call { float, i32 } @llvm.frexp.f32.i32(float [[X]])
; CHECK-NEXT:    [[EXP:%.*]] = extractvalue { float, i32 } [[FREXP]], 1
; CHECK-NEXT:    ret i32 [[EXP]]
